...
public class CosmosVec2 implements Comparable<CosmosVec2> {
	...
	public boolean equals(Object objectIn) {
		if (this == objectIn) {
			return true;
		} else if (!(objectIn instanceof CosmosVec2)) {
			return false;
		} else {
			CosmosVec2 vector3i = (CosmosVec2) objectIn;
...
//		 return new CosmosVec2(this.getZ() * vec.getZ() - this.getZ() * vec.getZ(), this.getZ() * vec.getX() - this.getX() * vec.getZ(), this.getX() * vec.getZ()- this.getZ() * vec.getX());
//	}
